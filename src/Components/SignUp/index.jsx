...
const SignUp = () => {
  ...
  const registerUser = async (data) => {
    ...
    noAuthAxiosHelper
      .post("/register", data)
      .then((response) => {
        toast({ title: "Registration Successfull", status: "success" });
        console.log(response);
        localStorage.setItem("onboardName", data.name);
        localStorage.setItem("cfl_id", response.data.body._id);
        setIsLoading(false)
        navigate("/onboarding");
      })
      .catch((error) => {
  ...
  return (
    <section className="signUpMain flex justify-center items-center absolute bg top-0 left-0 right-0 bottom-0">
      <div className="formWrapper sm:mx-auto sm:w-full rounded-3xl md:py-10 sm:py-9 lg:py-6  py-8 md:px-12 sm:px-7 px-4 relative z-10" style={{ display: forgetPsswordOpen ? "none" : "block" }}>
        ...
        <Image width={"300px"} margin={"50px auto"} paddingLeft={"3rem"} src="/assets/images/newLogo.png" />
        <HStack flexDirection={"column"} width={"450px"} backgroundColor={"#ffff"} padding={"20px 25px"} position={"relative"} borderRadius={"20px"} margin={"auto"} border={"1px solid #CEE0FF"}>
          ...
          <Button border={"1px solid #CEE0FF"} color={"#0080FF"} backgroundColor={"#E7F0FF"} borderRadius={"20px"} position={"absolute"} top={"-20px"}>Don’t have an account?</Button>
          <form className="space-y-6 w-full" onSubmit={handleSubmit(registerUser)}>
            ...
            <Box>
              <Button type="submit" backgroundColor={"#0080FF"} color={"#FFFFFF"} fontWeight={"bold"} borderRadius={"20px"} _hover={{ color: "black", backgroundColor: "white" }} width={"110px"} justifyContent={"space-around"} display={"flex"}>
                {isLoading ? <Spinner color='red.500' /> : <>Sign Up <FiArrowUpRight /></>}
              </Button>
            </Box>

            <Box>
              <Text cursor={"pointer"} fontWeight={"400"}>Do you have an Account? <span style={{ color: "#0080FF", fontWeight: "bold" }} onClick={() => navigate("/login")}>Sign In</span></Text>
            </Box>
          </form>
        </HStack>
      </div>
